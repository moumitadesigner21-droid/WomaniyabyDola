...
- `customers/` — storefront accounts: `repository.ts` (customers, `customer_addresses`, `wishlist_items`, admin stats), `session.ts` (`getCurrentCustomer()`, cookie `womania_customer_session`, HMAC key derived from `CUSTOMER_SESSION_SECRET` → `ADMIN_SESSION_SECRET`), `schemas.ts`. Orders carry `customer_id` when placed signed-in; `/api/account/*` routes are customer-scoped. Client state lives in `lib/customer.tsx` (`useCustomer()`: session, addresses, wishlist with a localStorage guest fallback merged on login). Protected pages sit in `app/account/(protected)/`; `/account/wishlist`, `/account/login`, `/account/register` are public.

...
export function CategoryRail({ items, label }: { items: RailItem[]; label: string }) {
  ...
  return (
    <nav aria-label={label} className="-mx-4 px-4 sm:mx-0 sm:px-0">
      <ul className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none sm:flex-wrap sm:overflow-visible sm:pb-0">
        {items.map((item) => (
          <li key={item.key} className="shrink-0">
            <Link
              ...
              className={`flex h-11 items-center gap-2.5 whitespace-nowrap rounded-full border pr-4 text-xs tracking-wide transition-colors ${
            ...
            >
              {item.image ? (
                <span className="relative h-8 w-8 overflow-hidden rounded-full bg-ivory">
                  <Image src={item.image} alt={item.label} fill className="object-cover" sizes="32px" />
                </span>
              ) : null}
